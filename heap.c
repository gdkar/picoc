...
# endif
#endif
    while (((unsigned long)&pc->HeapMemory[AlignOffset] & (sizeof(ALIGN_TYPE)-1)) != 0)AlignOffset++;
...
#endif
    if (NewTop > (char *)pc->HeapBottom) return NULL;
...
int HeapPopStack(Picoc *pc, void *Addr, int Size){
    ...
    if (ToLose > ((char *)pc->HeapStackTop - (char *)&(pc->HeapMemory)[0])) return FALSE;
#ifdef DEBUG_HEAP
...
#endif
        if ((char *)pc->HeapBottom - AllocSize < (char *)pc->HeapStackTop)return NULL;
